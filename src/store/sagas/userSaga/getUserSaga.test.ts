...
describe('getUserSaga tests', () => {
    ...
    test('test success case', async () => {
        ...
        await runSaga(fakeStore, getUser, {type: type, payload: 1},).toPromise()
        expect(spy).toBeCalledTimes(1);
        expect(dispatched.length).toEqual(2);
        expect(dispatched).toContainEqual(userActions.setUserLoading())
        expect(dispatched).toContainEqual(userActions.setUser(...response.data))
    })
    ...
    test('test error case', async () => {
        ...
        expect(dispatched.length).toEqual(2);
        expect(dispatched).toContainEqual(userActions.setUserLoading())
        expect(dispatched).toContainEqual(userActions.setUserError(responseWithError.data.message))
