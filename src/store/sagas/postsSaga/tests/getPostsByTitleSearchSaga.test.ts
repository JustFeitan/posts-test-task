...
jest.mock("axios");
describe("getPostsByTitleSearchSaga tests", () => {
    ...
    test("test success case", async () => {
        const spy = jest.spyOn(postsService, "getPostsByTitleSearch");
        spy.mockReturnValue(Promise.resolve(response as AxiosResponse<Post[]>));

        await runSaga(fakeStore, getPostsByTitleSearchSaga, {
            type: type,
            payload: { page: 1 },
        }).toPromise();
        ...
        expect(dispatched).toContainEqual(
            postsActions.setPagesArray(pagesArray)
        );
    });
    ...
    test("test error case", async () => {
        ...
        expect(dispatched).toContainEqual(postsActions.setPostsLoading());
        expect(dispatched).toContainEqual(
            postsActions.setPostsError(responseWithError.data.message)
        );
    });
    afterEach(() => {
        clearAllMocks();
