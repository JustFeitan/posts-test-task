...
describe("comments slice tests", () => {
    ...
    test("should handle setting posts loading", () => {
        expect(
            postsReducer(initialPostsSate, postsActions.setPostsLoading)
        ).toEqual({ ...initialPostsSate, isLoading: true });
    });

    test("should handle setting posts", () => {
        expect(
            postsReducer(initialPostsSate, postsActions.setPosts(postsMock))
        ).toEqual({ ...initialPostsSate, posts: postsMock });
    });

    test("should handle setting posts error", () => {
        expect(
            postsReducer(initialPostsSate, postsActions.setPostsError("Error"))
    ...
    test("should handle setting posts pagesArray", () => {
        expect(
            postsReducer(
                initialPostsSate,
                postsActions.setPagesArray([1, 2, 3])
            )
    ...
    test("should execute getPostsByTitleSearch for trigger saga", () => {
        expect(
            postsReducer(
                initialPostsSate,
                postsActions.getPostsByTitleSearch({ page: 1 })
            )
